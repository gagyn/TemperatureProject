...
class ConfigurationService:
    ...
    def get_records_count(self) -> int:
        recordsCount = self._get_configuration_by_name('recordsCount')
    ...
    def get_records_seconds_between(self) -> int:
        secondsBetween = self._get_configuration_by_name('secondsBetween')
    ...
    def get_arduino_port(self) -> str:
        port = self._get_configuration_by_name('port')
        if port is None:
    ...
    def get_reading_state(self) -> str:
        state = self._get_configuration_by_name('readingState')
        if state is None:
            return 'stopped'
        return state['value']
    ...
    def _get_configuration_by_name(self, configurationName: str) -> dict:
        return self._mongoClient.db.configurations.find_one({'name': configurationName})
